...
class MovieGenresDataloader : MappedBatchLoader<Int, List<Genre>> {
    override fun load(keys: Set<Int>) = CompletableFuture.supplyAsync { genresByMovie(keys) }!!

    private fun genresByMovie(movieIds: Set<Int>): Map<Int, List<Genre>> = transaction {
        ...
        movieIds.associateWithOrDefaultEmpty { foundMovieGenres[it].orEmpty() }
    }
}